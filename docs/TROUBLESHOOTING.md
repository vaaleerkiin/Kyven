...
Kyven API 20 uses port `18783`. Older development servers may remain on 8765-8769 or 18768-18782,
but the adapter asks authenticated older servers to unload their models before starting API 20.
...
authenticated Kyven service on port `18783`; it does not terminate Nuke or unrelated Python tools.
...
- After updating from an older API, restart Nuke so it launches the server on port 18783.
