...
Kyven API 19 uses port `18782`. Older development servers may remain on 8765-8769 or 18768-18781,
but the adapter asks authenticated older servers to unload their models before starting API 19.
...
authenticated Kyven service on port `18782`; it does not terminate Nuke or unrelated Python tools.
...
- After updating from an older API, restart Nuke so it launches the server on port 18782.
