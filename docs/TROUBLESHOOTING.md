...
Kyven API 17 uses port `18780`. Older development servers may remain on 8765-8769 or 18768-18779,
but the adapter asks authenticated older servers to unload their models before starting API 17.
...
authenticated Kyven service on port `18780`; it does not terminate Nuke or unrelated Python tools.
...
- After updating from an older API, restart Nuke so it launches the server on port 18780.
