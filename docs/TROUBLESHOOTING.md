...
Kyven API 18 uses port `18781`. Older development servers may remain on 8765-8769 or 18768-18780,
but the adapter asks authenticated older servers to unload their models before starting API 18.
...
authenticated Kyven service on port `18781`; it does not terminate Nuke or unrelated Python tools.
...
- After updating from an older API, restart Nuke so it launches the server on port 18781.
